...
test = c(3, 7, 8, 9, 14, 16, 17)

run.results = matrix(0, nrow = 8, ncol=24)
...
for (num in 0:1) {
  ...
  for (ci in 1:ncol(combos)) {
    ...
    if (17 %in% vars){
      run.results[index, 7] = 1
    }
    
    
    run.results[index, 8:ncol(run.results)] = all_methods(subset)
    index = index + 1
  }
...
#save(run.results, file='group4_variable_test_results.RData')

run.ranks = cbind(run.results[, 1:7], apply(apply(run.results[, -c(1:7)], 2, order), 2, order))
