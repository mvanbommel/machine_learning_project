...
for (i in 1:10) {
  ...
  test.set = data[which(cv.labels == i), -21]
  training.set = data[which(cv.labels != i), -21]
  
  y.1.df = training.set[, 21]
  x.1.df = training.set[, -21]
  y.2.df = test.set[, 21]
  x.2.df = test.set[, -21]

  training.set.matrix = as.matrix(training.set)
  y.1.mat = training.set.matrix[, 21]
  x.1.mat = training.set.matrix[, -21]
  test.set.matrix = as.matrix(test.set)
  y.2.mat = test.set.matrix[, 21]
  x.2.mat = test.set.matrix[, -21]
  ...
  cv.lasso = cv.glmnet(y=y.1.mat, x= x.1.mat, family="gaussian", lambda=exp(seq(from=0, to=-5, length=200)))
  plot(cv.lasso)
  ...
  # Random Forest
  ###
  rf.model = randomForest(data=training.set, Y~., importance=TRUE, ntree=500, mtry=14, nodesize=10, keep.forest=TRUE)
...
#save(test.error, file='initial_method_comparison.RData')
load('initial_method_comparison.RData')
...
boxplot(x=root.test.error.scaled, main="Relative Root MSPEs", 
        xlab="Variable Selection Method", ylab="Root MSPE relative to best")
