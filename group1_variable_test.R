...
run.results = matrix(0, nrow = 15, ncol=21)

index = 1
for (num in 1:4) {
  combos = combn(include, num)
  ...
  for (ci in 1:ncol(combos)) {
    ...
    if (num == 1) {
      run.results[index, 5:ncol(run.results)] = all_methods_1(subset)
    } else {
      run.results[index, 5:ncol(run.results)] = all_methods(subset)
...
#save(run.results, file='group1_variable_test_results.RData')

run.ranks = cbind(run.results[, 1:4], apply(apply(run.results[, -c(1:4)], 2, order), 2, order))


all_methods_1 = function(data) {
  set.seed(1)
  ...
  for (i in 1:5) {
    ...
    # OLS
    ###
    ols.model = lm(y.1.df ~ ., data = training.set)
    ols.predict.1 = predict(ols.model, newdata = training.set)
    ols.predict.2 = predict(ols.model, newdata = training.set)
    ...
    # Stepwise BIC
    ###
    initial.model = lm(y.1.df ~ 1, data = training.set)
    final.model = lm(y.1.df ~ ., data = training.set)
    stepwise.selection.model = step(object=initial.model, scope=list(upper=final.model), direction='both', k=log(nrow(training.set)), trace=0)
    stepwise.predict.1 = predict(stepwise.selection.model, newdata = training.set)
    stepwise.predict.2 = predict(stepwise.selection.model, newdata = training.set)
    ...
    # PPR
    ###
    ppr1.model = ppr(data=training.set, y.1.df ~ ., nterms=1, optlevel=3) 
    ppr2.model = ppr(data=training.set, y.1.df ~ ., nterms=2, optlevel=3) 
    ppr3.model = ppr(data=training.set, y.1.df ~ ., nterms=3, optlevel=3) 
    
    ppr1.predict.1 = predict(ppr1.model, newdata=training.set)
    ppr1.predict.2 = predict(ppr1.model, newdata=test.set)
    ppr2.predict.1 = predict(ppr2.model, newdata=training.set)
    ppr2.predict.2 = predict(ppr2.model, newdata=test.set)
    ppr3.predict.1 = predict(ppr3.model, newdata=training.set)
    ppr3.predict.2 = predict(ppr3.model, newdata=test.set)
