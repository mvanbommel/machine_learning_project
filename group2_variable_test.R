...
run.results = matrix(0, nrow = 8, ncol=20)
...
for (num in 1:3) {
  ...
  for (ci in 1:ncol(combos)) {
    ...
    run.results[index, 4:ncol(run.results)] = all_methods(subset)
    index = index + 1
  }
...
#save(run.results, file='group2_variable_test_results.RData')

run.ranks = cbind(run.results[, 1:3], apply(apply(run.results[, -c(1:3)], 2, order), 2, order))
